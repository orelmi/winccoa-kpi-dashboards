...
const OeeAnalysis = (() => {
  ...
  function _renderAnalysis(machine, stateStats, causeStats, oeeResult, tRange) {
    ...
    // ── OEE Gauges (if available) ─────────────────────────────
    if (oeeResult) {
      html += _renderOeeGauges(oeeResult);
    }
  ...
  function _renderOeeGauges(r) {
    ...
    return '<div class="analysis-section">' +
      ...
      '<p class="analysis-hint">Calculated on the fly from raw archive data for the selected period</p>' +
      '<div class="oee-gauges">' +
        _gauge('Availability', r.availability, '#17a2b8') +
        _gauge('Performance', r.performance, '#6f42c1') +
        _gauge('Quality', r.quality, '#28a745') +
        _gauge('OEE', r.oee, '#009999') +
      '</div>' +
      '<div class="oee-time-breakdown">' +
  ...
  function _gauge(label, value, color) {
    ...
    return '<div class="oee-gauge">' +
      '<svg viewBox="0 0 100 100" class="gauge-svg">' +
        ...
        '<text x="50" y="48" text-anchor="middle" class="gauge-value" fill="' + color + '">' + displayPct + '%</text>' +
  ...
  function init() {
    ...
    if (machineSelect) machineSelect.addEventListener('change', refresh);
